...
class Chapter6Synthesis(ModulePage):
    ...
    def _export(self):
        N = self.N.value(); d = self.d_slider.value() / 100.0; sll = self.sll_slider.value()
        th = np.linspace(0, np.pi, 2000)
        u = F.array_factor_uniform(th, N, d, 0.0)
        b = F.array_factor_uniform(th, N, d, 0.0, taper=F.binomial_coeffs(N))
        dc, _x0, _R0 = F.dolph_tschebyscheff_coeffs(N, sll)
        dd = F.array_factor_uniform(th, N, d, 0.0, taper=dc)
        return (["theta_deg", "uniform", "binomial", "dolph"],
                list(zip(np.degrees(th), u, b, dd)))
