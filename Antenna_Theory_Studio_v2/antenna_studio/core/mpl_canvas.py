...
class ModulePage(QWidget):
    ...
    # βοηθητικό: κουμπί εξαγωγής δεδομένων σε CSV
    def add_export_button(self, data_fn, label="⤓ Εξαγωγή CSV"):
        """data_fn() -> (header:list[str], rows:list[tuple]).  Προσθέτει κουμπί
        που ανοίγει διάλογο αποθήκευσης και γράφει CSV. (PNG/PDF από την toolbar.)"""
        ...
        def _save():
            try:
                ...
                return
            path, _ = QFileDialog.getSaveFileName(self, "Αποθήκευση CSV", "data.csv",
                                                  "CSV (*.csv)")
            if not path:
                return
